...
/**
 * Today's scoreboard: every member ranked by the points they earned *this
 * matchday only* (not the overall table). Top three get medals, everyone else a
 * number, and the day's worst sit at the bottom (a plain ۰ when they got
 * nothing). Rows stagger in. Reduced-motion shows them at rest.
 */
function DayScoreboard({ rows }: { rows: RecapScoreRow[] }) {
  // Backend already orders by today's points; keep it stable on the client too.
  const ordered = [...rows].sort((a, b) => b.day_points - a.day_points || b.total - a.total);
  return (
    <>
      <p className="recap-eyebrow">تابلوی امتیاز روز</p>
      <p className="recap-sub">رتبه‌بندی بر اساس امتیاز همین روز</p>
      <div className="recap-sb">
        {ordered.map((r, i) => (
          <div
            key={r.id}
            className={`recap-sb-row ${r.is_me ? "me" : ""} ${r.day_points <= 0 ? "zero" : ""}`}
          >
            <span className="recap-sb-rank">{["🥇", "🥈", "🥉"][i] ?? fa(i + 1)}</span>
            <Avatar player={r} size={30} />
            <span className="recap-sb-name">
              {r.name}
              {r.is_me && <span className="muted"> (تو)</span>}
            </span>
            <span className="recap-sb-pts">{fa(r.day_points)}</span>
          </div>
        ))}
      </div>
    </>
...
/** A finished fixture as a one-line caption, e.g. «آلمان ۷:۱ کوراسائو». */
function matchLabel(m: RecapMatchMini): string {
  const h = m.home_team?.name ?? m.home_label ?? "؟";
  const a = m.away_team?.name ?? m.away_label ?? "؟";
  return `${h} ${fa(m.home_score ?? "")}:${fa(m.away_score ?? "")} ${a}`;
}

// Pacing of the match-by-match replay (ms): the first reveal, each match step,
// and the final settle where ▲▼ markers appear.
const STEP_FIRST = 1400;
const STEP_MATCH = 1900;
const STEP_SETTLE = 1200;

/**
 * The overall table, replayed match by match. It opens on *yesterday's* table,
 * then applies the day's matches one at a time — after each result the rows
 * slide to their new positions (Framer `layout`), and each row carries a ▲▼
 * marker showing how far it has moved **versus the start-of-day table** (not the
 * last match), updating as matches accumulate. Steps:
 *   0            -> table before the day ("yesterday"), no markers
 *   1..n         -> after match k (n = number of matches), markers vs day-start
 *   n+1          -> a final beat captioned "end of today"
 * Reduced-motion jumps straight to the final table.
 */
function StandingsShuffle({
  rows,
  matches,
}: {
  rows: RecapScoreRow[];
  matches: RecapMatchMini[];
}) {
  const reduce = useReducedMotion();
  ...
  useEffect(() => {
    if (reduce || step > n) return;
    const delay = step === 0 ? STEP_FIRST : step === n ? STEP_SETTLE : STEP_MATCH;
    const t = setTimeout(() => setStep((s) => s + 1), delay);
    return () => clearTimeout(t);
  }, [step, n, reduce]);

  const applied = Math.min(step, n); // matches counted into the table so far
  // Cumulative total at this step, sorted, with tie-aware ranks (ties share a
  // rank, like the official table). Ranks are recomputed each step so the ▲▼
  // movement is always measured against the start-of-day table — not the
  // previous match.
  const sorted = rows
    .map((r) => ({
      r,
      // Round to cents to match the backend's Decimal points: summing floats
      // (e.g. with a non-half stage multiplier) would otherwise drift and both
      // mis-tie the ranks and show an ugly long decimal.
      total: Math.round(
        (r.total_before + r.match_points.slice(0, applied).reduce((a, b) => a + b, 0)) * 100,
      ) / 100,
    }))
    .sort((a, b) => b.total - a.total || b.r.total_before - a.r.total_before);
  let rank = 0;
  let prevTotal = Number.NaN;
  const standings = sorted.map((row, i) => {
  ...
  const caption =
    step === 0
      ...
      : step > n
        ? "جدول در پایان امروز"
        : `بعد از ${matchLabel(matches[step - 1])}`;

  return (
    <>
      <p className="recap-eyebrow">جابه‌جایی جدول</p>
      <p className="recap-sub">{caption}</p>
      <div className="recap-sb">
        {standings.map(({ r, total, rank }) => {
          // Movement vs the start-of-day table, shown from the first match on.
          const move = step >= 1 ? r.rank_before - rank : 0;
          return (
            <motion.div
              layout
              key={r.id}
              transition={{ type: "spring", stiffness: 320, damping: 32 }}
              className={`recap-sb-row ${r.is_me ? "me" : ""}`}
            >
              <span className="recap-sb-rank">{fa(rank)}</span>
              <Avatar player={r} size={30} />
              <span className="recap-sb-name">
                {r.name}
                {r.is_me && <span className="muted"> (تو)</span>}
              ...
              {move !== 0 && (
                <span className={`rank-move ${move > 0 ? "up" : "down"}`}>
                  {move > 0 ? "▲" : "▼"}
                  {fa(Math.abs(move))}
                </span>
              )}
              <span className="recap-sb-total">{fa(total)}</span>
            </motion.div>
          );
        })}
      </div>
...
function buildCards(recap: RecapResp, slug: string): Card[] {
  ...
  // 9) Today's scoreboard — ranked by points earned this matchday only.
  if (recap.scoreboard.length) {
    cards.push({
      key: "scoreboard",
      tone: "rank",
      node: <DayScoreboard rows={recap.scoreboard} />,
    });
    // 10) The overall table reshuffling from yesterday to end-of-today.
    cards.push({
      key: "shuffle",
      tone: "blue",
      node: <StandingsShuffle rows={recap.scoreboard} matches={recap.matches} />,
    });
  }

  // 11) Closing podium — current standings, with a link to the full table.
...
export default function RecapStory({
  ...
  // Arrow keys page the story (RTL: ← advances, → goes back). Space/Enter are
  // left to the focused control so activating a button/dot doesn't double-advance.
  useEffect(() => {
