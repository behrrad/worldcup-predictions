...
match finished. For one league and one matchday this module computes:
  ...
  * a personal summary for the viewer (points, hit breakdown, best call, and
    how their leaderboard rank moved across the day),
  * league-wide superlatives (top scorer, best single call, the day's biggest
    upset, the biggest rank climber and faller) plus the standings after the day, and
  * the full day scoreboard — every member with their day points and rank
    move (start-of-day -> end-of-day), for the reordering animation.
...
_ZERO = Decimal("0.00")
_MATCHDAY_TZ = ZoneInfo(consts.RECAP_MATCHDAY_TZ)
...
def _local_date(dt):
    """The calendar date of a kickoff in the schedule's timezone.

    Uses the host region's zone (consts.RECAP_MATCHDAY_TZ), not the viewer's, so
    a matchday's whole slate stays on one date — see the constant's note.
    """
    return dt.astimezone(_MATCHDAY_TZ).date()
...
def build_recap(league, viewer_membership, date_str=None):
    ...
    if not dates:
        return {"date": None, "available_dates": [], "matches": [],
                "me": None, "general": None, "scoreboard": []}
    ...
    return {
        ...
        "me": me,
        "general": general,
        "scoreboard": _scoreboard(memberships, rank_before, rank_after, before, after,
                                  day_points, day_scores, day_matches),
    }


def _scoreboard(memberships, rank_before, rank_after, before, after, day_points,
                day_scores, day_matches):
    """Today's scoreboard — every member ranked by the points they earned *this
    matchday only* (not the cumulative table). The best of the day are on top,
    the worst at the bottom.

    Each row also carries enough to animate the overall table match by match:
    `total_before` (the cumulative table before the day) and `match_points` —
    the points the member earned on each of the day's matches, in kickoff order
    (aligned with the recap's `matches`). The UI starts from `total_before` and
    adds one match at a time, reshuffling the standings after each.
    """
    rows = [
        {
            "membership": m,
            "rank_before": rank_before[m.id],
            "rank_after": rank_after[m.id],
            "total": after.get(m.id, _ZERO),
            "total_before": before.get(m.id, _ZERO),
            "day_points": day_points.get(m.id, _ZERO),
            "match_points": [
                day_scores[(m.id, mm.id)].points if (m.id, mm.id) in day_scores else _ZERO
                for mm in day_matches
            ],
        }
        for m in memberships
    ]
    # Most points today first; ties broken by the cumulative table, then join order.
    rows.sort(key=lambda r: (-r["day_points"], -r["total"], r["membership"].joined_at))
    return rows
...
def _general_recap(memberships, day_matches, day_match_ids, summaries, day_scores,
    ...
    best_key = None
    for m in memberships:
        # Consider every call the member made today, not just their highest —
        # a same-points but rarer call should win on the rarity tiebreak below.
        for s, match, pred in summaries[m.id]["predicted"]:
    ...
    # Biggest climber and biggest faller: largest rank change each way.
    mover = faller = None
    best_up = best_down = 0
    for m in memberships:
        delta = rank_before[m.id] - rank_after[m.id]  # +ve = climbed
        if delta > best_up:
            best_up = delta
            mover = {"membership": m, "from_rank": rank_before[m.id],
                     "to_rank": rank_after[m.id], "delta": delta}
        elif -delta > best_down:
            best_down = -delta
            faller = {"membership": m, "from_rank": rank_before[m.id],
                      "to_rank": rank_after[m.id], "delta": -delta}
    ...
    return {"top_scorer": top_scorer, "best_call": best_call,
            "surprise": surprise, "mover": mover, "faller": faller, "podium": podium}
